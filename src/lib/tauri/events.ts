...
export async function listenTaskEvents(
  ...
  const unlistenProgress = await listen<TaskEventPayload>(
    TASK_PROGRESS_EVENT,
    (event) => handler(event.payload),
  );
  const unlistenCompleted = await listen<TaskEventPayload>(
    TASK_COMPLETED_EVENT,
    (event) => handler(event.payload),
  );
  const unlistenFailed = await listen<TaskEventPayload>(
    TASK_FAILED_EVENT,
    (event) => handler(event.payload),
  );
...
export async function listenAppReady(
