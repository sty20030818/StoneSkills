...
describe('GithubImportPreviewPanel', () => {
	it('会高亮技能数量，并阻止禁用候选项进入选择流程', () => {
		...
		expect(screen.getByTestId('github-import-preview-root')).toHaveClass('gap-3')
		...
		expect(screen.getByTestId('github-import-preview-list-items')).toHaveClass('gap-3')
		expect(screen.getByTestId('github-import-preview-actions')).toHaveClass('shrink-0')
		expect(screen.getByTestId('github-import-preview-actions')).toHaveClass('pb-3')
		expect(screen.getByTestId('github-import-preview-actions')).not.toHaveClass('pb-4')
