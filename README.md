...
当前仓库已完成模块 1 的基础骨架搭建，并补上了模块 2 的本地数据层，重点是：
...
- Zustand 状态切片骨架
- 基础错误处理与日志入口
- `rusqlite + rusqlite_migration` 本地 SQLite 状态库
- Skill / Target / Installation / Settings 统一数据模型
...
- `src-tauri/src/database`：SQLite 连接初始化与 migration
- `src-tauri/src/commands`：IPC 命令边界
- `src-tauri/src/repositories`：SQLite 读写与行映射
- `src-tauri/src/services`：系统路径、日志、文件等基础服务
- `src-tauri/src/platform`：macOS / Windows 平台差异抽象
- `src-tauri/src/models`：Domain、DTO 与统一返回模型
...
- `list_targets` / `upsert_target`
- `list_installations` / `upsert_installation`
- `get_app_settings_snapshot` / `set_app_setting`
...
- 模块 2 只实现本地数据模型与存储底座，尚未接入真实的 GitHub 安装、本地扫描、更新与分发流程
